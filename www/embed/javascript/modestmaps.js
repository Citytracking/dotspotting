...
(function(MM) {
    ...
    MM.Location.fromString = function(str, lonFirst) {
        var parts = str.split(/\s*,\s*/);
        parts[0] = parseFloat(parts[0]);
        parts[1] = parseFloat(parts[1]);
        if (parts.length == 2) {
            return lonFirst
                ? new MM.Location(parts[0], parts[1])
                : new MM.Location(parts[1], parts[0]);
        } else {
            return null;
        }
    };
    ...
    */
    ...
    MM.Layer = function(map, provider, parent)
    {
        ...
        this.parent.style.left = this.parent.style.right = "0px";
        this.parent.style.width = this.parent.style.height = "100%";
    ...
    MM.Map.prototype = {
        ...
        removeProvider: function(provider) {
            ...
            for (var i = 0; i < len; i++) {
                if (this.layers[i] == provider || this.layers[i].template_provider == provider) {
                    layer = this.layers[i];
                    index = i;
                    break;
                }
            ...
            if (index > -1) {
                ...
                layer.parent.parentNode.removeChild(layer.parent);
                return true;
            } else {
                return false;
            }
