...
po.queue = (function() {
  ...
  /*
	 * Our JSONP function pollutes the JavaScript window namespace with a
	 * uniquely named function that wraps the callback and cleans up after
	 * itself by deleting the global function and removing the <script> element
	 * from the DOM.
   */

	var head = document.getElementsByTagName("head")[0] || document.documentElement;
	function jsonp(url, callback, param) {
		if (!param) param = 'callback';
		var cb = jsonp.prefix + (++jsonp.id),
  ...
  return {text: text, xml: xml, json: json, jsonp: jsonp, image: image};
})();
po.map = function() {
  var map = {},
      container,
...
po.geoJson = function(fetch) {
  ...
  function applyAttrs(feature, node) {
    if (attrs) {
      for (var a in attrs) {
        var v = attrs[a];
        if (typeof v == "function") {
          v = v.call(geoJson, feature);
        }
        if (a == "title") {
          po.svg.title(node, v);
        } else {
