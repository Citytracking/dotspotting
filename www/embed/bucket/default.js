...
function validateUserColors(arr) {
    var len = arr.length,
        out = [];
    for(var i=0;i<len;i++) {
        if (checkHex(arr[i])) out.push(arr[i]);
    }
...
$(function() {
    // user settings
    // only color scale supported
    d3.select(window).on("message", function(){
        d3.select(window).on("message", null);
        var type = d3.event.message ? 'message' : 'data';
        var payload = d3.event[type];

        if (payload instanceof Object &&
            payload.hasOwnProperty('colors') &&
            payload.colors instanceof Array) {

            var c = validateUserColors(payload.colors);
            if (c.length) userSettings.colors = c;
        }
    }, false);
    ...
    try{
        ...
        try{
            window.parent.postMessage('userSettings', '*');
            setTimeout(function(){
                init();
            }, 500);
            ...
            var req = pot.load(null,function(){
                ...
                if (typeSelector) {
                    ...
                    if (userSettings.colors &&
                        userSettings.colors instanceof Array) {

                        colorScale = chroma.scale(userSettings.colors).domain([0, bucketCount], bucketCount);

                    } else if (typeof chroma !== 'undefined' &&
                            params.cs &&
                            chroma.brewer[params.cs]){

                        colorScale = chroma.scale(params.cs).domain([0, bucketCount], bucketCount);
                    }

                    if (!colorScale) {
                        colorScale = chroma.scale(d3.scale.category20().range()).domain([0, bucketCount], bucketCount);
                    }
                    ...
                    for (var i=0;i<bucketList.length;i++) {
                        var label = typeSelector.createBuckets(bucketList[i]);
                    }

                    typeSelector.makeLabels();

                    //typeSelector.addLabel(bucketList[i]);
...
MenuSelector.prototype = {
    ...
    createBuckets: function(data) {
        ...
        // check for label
        if (!this.buckets.hasOwnProperty(type)) {
            this.buckets[type] = 1;
            this.labelCounts[type] = 0;
        }
    ...
    markerShow: function(m) {
        ...
        m.attr("opacity", 1);
        m.attr("stroke-opacity", 1);
        m.attr("fill-opacity", 1);
    },
    markerHide: function(m) {
        m.attr("opacity", 0);
        m.attr("stroke-opacity", 0);
        m.attr("fill-opacity", 0);
    },
    ...
    highlightMarkers: function(t){
        ...
        for(i=0;i<len;i++){
            if(markers[i].myAttrs.id){
                ...
                var isActive = markers[i].myAttrs['props']['__active'],
                    isMatch = (t == markers[i].myAttrs['type']) ? true : false;

                if (isMatch) markers[i].toFront();
                if (isMatch && isActive) {
                    this.markerShow(markers[i]);
                } else {
                    this.markerHide(markers[i]);
                }
                ...
                if(t == markers[i].myAttrs['type']){
                    ...
                    if( markers[i].myAttrs['props']['__active']){
                        if(ds_kirby)backdict[markers[i].myAttrs.id].attr("opacity", 0.2);
                        markers[i].attr("opacity", 0.2);

                    } else {
                        markers[i].attr("stroke-opacity", 0);
    ...
    hideMarkers: function(t){
        ...
        for(i=0;i<len;i++){
            ...
            var isMatch = (t == markers[i].myAttrs['type']) ? true : false;
